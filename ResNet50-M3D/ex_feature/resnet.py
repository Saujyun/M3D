...
class Attention(nn.Module):
	...
	def __init__(self, inplanes = 16, frames = 4):
		super(Attention, self).__init__()

		self.frames = frames

		self.relu = nn.ReLU(inplace=True)
		self.conv1_s = nn.Conv2d(inplanes, 1, kernel_size=3, stride=1, padding=1, bias=False)
		self.conv2_s = nn.Conv2d(1, 1, kernel_size=1, bias=False)
		self.bn_s = nn.BatchNorm2d(1)
		n = self.conv1_s.kernel_size[0] * self.conv1_s.kernel_size[1] * self.conv1_s.out_channels
		self.conv1_s.weight.data.normal_(0, math.sqrt(2. / n))
		self.conv2_s.weight.data.fill_(0)

		self.conv1_c = nn.Conv2d(inplanes, inplanes/16, kernel_size=1, stride=1, bias=False)
		self.conv2_c = nn.Conv2d(inplanes/16, inplanes, kernel_size=1, stride=1, bias=False)
		self.conv2_c.weight.data.fill_(0)
		self.bn_c = nn.BatchNorm2d(inplanes/16)
		n = self.conv1_c.kernel_size[0] * self.conv1_c.kernel_size[1] * self.conv1_c.out_channels
		self.conv1_c.weight.data.normal_(0, math.sqrt(2. / n))		

		self.conv1_t = nn.Conv2d(frames, frames*2, kernel_size=1, stride=1, bias=False)
		self.conv2_t = nn.Conv2d(frames*2, frames, kernel_size=1, stride=1, bias=False)
		self.conv2_t.weight.data.fill_(0)
		self.bn_t = nn.BatchNorm2d(frames*2)
		n = self.conv1_t.kernel_size[0] * self.conv1_t.kernel_size[1] * self.conv1_t.out_channels
		self.conv1_t.weight.data.normal_(0, math.sqrt(2. / n))	
		
		self.sigmoid = nn.Sigmoid()

	def forward(self, x):
		#print x.size()
		x = x.view(x.size(0)/self.frames, self.frames, x.size(1), x.size(2),x.size(3))
		x = torch.transpose(x, 1, 2)

		s_mask = torch.mean(x,2)
		s_mask = self.conv1_s(s_mask)
		s_mask = self.relu(s_mask)
		s_mask = self.bn_s(s_mask)
		s_mask = self.conv2_s(s_mask)		
		
		c_mask = torch.mean(x,2)
		c_mask = c_mask.mean(2, keepdim=True).mean(3, keepdim=True)
		c_mask = self.conv1_c(c_mask)
		c_mask = self.relu(c_mask)
		c_mask = self.bn_c(c_mask)
		c_mask = self.conv2_c(c_mask)
		c_mask = self.relu(c_mask)
		
		t_mask = torch.mean(x,1)		
		t_mask = t_mask.mean(2, keepdim=True).mean(3, keepdim=True)
		t_mask = self.conv1_t(t_mask)
		t_mask = self.relu(t_mask)
		t_mask = self.bn_t(t_mask)
		t_mask = self.conv2_t(t_mask)
		t_mask = t_mask.unsqueeze(1)#.unsqueeze(3).unsqueeze(3)
...
class ResNet(nn.Module):
	...
	def __init__(self, block, layers, num_classes=1000, train=True):
		...
		self.attention1 = Attention(inplanes = 256, frames = 8)
		self.attention2 = Attention(inplanes = 512, frames = 4)
		self.attention3 = Attention(inplanes = 1024, frames = 2)
	...
	def forward(self, x):
		...
		x = self.layer1(x)
		x = x/2 + self.attention1(x)
		x = self.layer2(x)
		x = x/2 + self.attention2(x)
		x = self.layer3(x)
		x = x/2 + self.attention3(x)
		x = self.layer4(x)
